...
{
  # setting the directory and saving database
  setwd(dwfinrds) 
  saveRDS(BASE, "BASE_Essay_II.rds")
...
# importing aerodromo database
aerodromos = read_delim(url("https://raw.githubusercontent.com/jrtonin/thesis_data/main/data/parameter/AERODROMOS.csv"), 
                        delim = ";", escape_double = FALSE, 
                        locale = locale(encoding = "ISO-8859-1"), 
                        trim_ws = TRUE)
...
# reading databases
HAUSMAN = readRDS("HAUSMAN_instrument.rds")
ESTBAN = readRDS("ESTBAN_instrument.rds")
...
BASE1 = BASE %>%
  left_join(aerodromos %>% dplyr::select(code, code_city, capital, region), 
            by = c("air_dep" = "code")) %>%
  mutate(code_city_dep = code_city,
         capital_dep = capital,
         region_dep = region) %>% 
  dplyr::select(-code_city, -capital, -region) %>%
  left_join(aerodromos %>% dplyr::select(code, code_city, capital, region), 
            by = c("air_arr" = "code")) %>%
  mutate(code_city_arr = code_city,
         capital_arr = capital,
         region_arr = region) %>% 
  dplyr::select(-code_city, -capital, -region) %>%
  left_join(ESTBAN %>% 
  ...
  mutate(ldep_nbank = nbank,
         ldep_deposits = deposits,
         ldep_credit_loan = credit_loan) %>% 
  ...
  mutate(larr_nbank = nbank,
         larr_deposits = deposits,
         larr_credit_loan = credit_loan) %>% 
  dplyr::select(-nbank, -deposits, -credit_loan) %>%
  ...
  mutate(m150RouteHHI = case_when(m150RouteHHI == 0 ~ RouteHHI, TRUE ~ m150RouteHHI),
         m300RouteHHI = case_when(m300RouteHHI == 0 ~ RouteHHI, TRUE ~ m300RouteHHI),
         m500RouteHHI = case_when(m500RouteHHI == 0 ~ RouteHHI, TRUE ~ m500RouteHHI),
         m150DepHHI = case_when(m150DepHHI == 0 ~ DepHHI, TRUE ~ m150DepHHI),
         m300DepHHI = case_when(m300DepHHI == 0 ~ DepHHI, TRUE ~ m300DepHHI),
         m500DepHHI = case_when(m500DepHHI == 0 ~ DepHHI, TRUE ~ m500DepHHI),
         m150seat_dem = case_when(m150seat_dem == 0 ~ seat_dem, TRUE ~ m150seat_dem),
         m300seat_dem = case_when(m300seat_dem == 0 ~ seat_dem, TRUE ~ m300seat_dem),
         m500seat_dem = case_when(m500seat_dem == 0 ~ seat_dem, TRUE ~ m500seat_dem),
         m150price = case_when(m150price == 0 ~ price, TRUE ~ m150price),
         m300price = case_when(m300price == 0 ~ price, TRUE ~ m300price),
         m500price = case_when(m500price == 0 ~ price, TRUE ~ m500price),
         m150RouteShare = case_when(m150RouteShare == 0 ~ RouteShare, TRUE ~ m150RouteShare),
         m300RouteShare = case_when(m300RouteShare == 0 ~ RouteShare, TRUE ~ m300RouteShare),
         m500RouteShare = case_when(m500RouteShare == 0 ~ RouteShare, TRUE ~ m500RouteShare))
  
    # transforming inf to 0
    is.na(BASE1) = sapply(BASE1, is.infinite)
    BASE1[is.na(BASE1)] = 0
    ...
    # filtering covid database
    LBASE1c = LBASE1 %>%
      filter(covid == TRUE)
    
} # organizing BASE1 and LBASE1

{
  # pre-organizing BASE2 database
    BASE2 = BASE1 %>% filter(N == 2)
    ...
    BASE2 = BASE2 %>% mutate(seat_offer = seat_offer.x - seat_offer.y,
                             ...
                             m150RouteHHI = m150RouteHHI.x - m150RouteHHI.y, 
                             m300RouteHHI = m300RouteHHI.x - m300RouteHHI.y,
                             m500RouteHHI = m500RouteHHI.x - m500RouteHHI.y,
                             m150DepHHI = m150DepHHI.x - m150DepHHI.y,
                             m300DepHHI = m300DepHHI.x - m300DepHHI.y,
                             m500DepHHI = m500DepHHI.x - m500DepHHI.y,
                             m150seat_dem = m150seat_dem.x - m150seat_dem.y,
                             m300seat_dem = m300seat_dem.x - m300seat_dem.y,
                             m500seat_dem = m500seat_dem.x - m500seat_dem.y,
                             m150price = m150price.x - m150price.y,
                             m300price = m300price.x - m300price.y,
                             m500price = m500price.x - m500price.y,
                             m150RouteShare = m150RouteShare.x - m150RouteShare.y,
                             m300RouteShare = m300RouteShare.x - m300RouteShare.y,
                             m500RouteShare = m500RouteShare.x - m500RouteShare.y) %>%
      group_by(year, month, air_arr, air_dep) %>%
      select(year, month, air_arr, air_dep,
             ...
             `0-5`, `6-11`, `12-17`, `18-23`, RouteShare,
             m150RouteHHI, m300RouteHHI, m500RouteHHI, 
             m150DepHHI, m300DepHHI, m500DepHHI,
             m150seat_dem, m300seat_dem, m500seat_dem,
             m150price, m300price, m500price,
             m150RouteShare, m300RouteShare, m500RouteShare)
    
    # editing the CONTROL database (basic information of routes)
    CONTROL = BASE1 %>% group_by(year, month, air_arr, air_dep) %>% 
    ...
    BASE2 = BASE2 %>% 
      ...
      # pre-organizing LBASE2 database
    ...
    # editing the CONTROL database (basic information of routes)
    CONTROL = LBASE1 %>% group_by(year, month, air_arr, air_dep) %>% 
      dplyr::summarise(FreqHHI = mean(FreqHHI),
                       RouteHHI = mean(RouteHHI),
                       ArrHHI = mean(ArrHHI),
                       DepHHI = mean(DepHHI),
...
} # organizing BASE2 and LBASE2
    
{
  # setting the directory and saving databases
  setwd(dwfinrds)
  saveRDS(BASE1, "BASE1_Essay_II.rds")
  saveRDS(LBASE1, "LBASE1_Essay_II.rds")
  saveRDS(LBASE1c, "LBASE1c_Essay_II.rds")
  saveRDS(LBASE2, "LBASE2_Essay_II.rds")
  saveRDS(LBASE2c, "LBASE2c_Essay_II.rds")
